...
# VERSION 1.0.1
# Edited risunCode at 2025-14-05
...
def prompt_network(self) -> str:
    ...
    if not networks:
        ani(f"{RED}[-]{WHITE} No WPS networks found!")
        ani(f"{YELLOW}[i]{WHITE} Turn on Wi-Fi briefly, then off.")
        ani(f"{YELLOW}[i]{WHITE} Turn on your hotspot if it’s off.")
        ani(f"{YELLOW}[i]{WHITE} Turn off location if it’s on.")

        for i in range(5, 0, -1):
            sys.stdout.write(f"\r{YELLOW}[+]{WHITE} Auto restarting in {YELLOW}[{i}]{WHITE} seconds...")
            sys.stdout.flush()
            time.sleep(1)
        print()
    ...
    while True:
        try:
            print(f"\n{GREEN}{'═' * 56}{RESET}")
            networkNo = input(f"{YELLOW}[?]{WHITE} Select target (press Enter to refresh): ")
            ...
            print(f"{RED}[!]{WHITE} Invalid number")
        ...
        try:
            ...
            if args.loop:
                if input("\n[?] Exit the script (otherwise continue to AP scan)? [N/y] ").lower() == 'y':
                    ani("\n[\033[1;31m!\033[1;37m] Proccess Exiting")
                    break
                else:
                    args.bssid = None
            else:
